...
function ServicesPage({ role }) {
    ...
    useEffect(() => {
        ...
        if (room) {
            const residents = room.rentalTimes.map(rt => rt.resident).filter(Boolean);
    ...
    const handleBookService = async (e, idService) => {
        ...
        const formData = new FormData(e.target);
        const booking = {
            quantity: Number(formData.get('duration')),
    ...
    const ServiceForm = ({ type, data = {}, onSubmit, onCancel }) => (
        <div className="svc-modal-overlay">
            <div className="svc-modal">
                <form className="svc-service-form" onSubmit={onSubmit}>
                    ...
                    {type === 'book' && (
                        <>
                            <div className="svc-form-group">
                                ...
                                <select value={selectedRoomId} onChange={e => setSelectedRoomId(Number(e.target.value))} required>
                            ...
                            <div className="svc-form-group">
                                <label htmlFor="duration">Số lượng</label>
    ...
    return (
        <div className="page svc-services-page">
            <h2>Dịch Vụ</h2>
            <button className="svc-add-button" onClick={() => setModal({ type: 'add' })}>+</button>

            <div className="svc-service-container">
                {services.map((service) => (
                    <div key={service.id} className="svc-service-card">
                        ...
                        <div className="svc-service-actions">
                            <button className="svc-edit-button" onClick={() => setModal({ type: 'edit', data: service })}>Sửa</button>
                            <button className="svc-delete-button" onClick={() => handleDeleteService(service.id)}>Xóa</button>
                            <button className="svc-book-button" onClick={() => setModal({ type: 'book', data: service })}>
                                Đặt Dịch Vụ
                            </button>
                        </div>
                    </div>
